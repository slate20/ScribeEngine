...
if __name__ == '__main__':
    # Parse command-line arguments for app.py itself
    parser = argparse.ArgumentParser(description='PyVN Flask App Server')
    parser.add_argument('--production', action='store_true', 
                        help='Run in production mode (debug=False, reloader=False)')
    parser.add_argument('--project', '-p', type=str, 
                        help='Path to the game project directory (e.g., game/my_game)',
                        default=os.path.join(os.path.dirname(__file__), 'game'))
    app_args = parser.parse_args()

    # Set the game project path based on the argument
    set_game_project_path(app_args.project)

    debug_mode = not app_args.production
    use_reloader = not app_args.production
