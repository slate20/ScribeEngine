...
from v1_engine.game_server import app as game_app, set_game_project_path
from shared.utils.loading_window import LoadingWindow

def start_game_server():
    """Start the game Flask server."""
    game_app.run(host='127.0.0.1', port=5001, debug=False, use_reloader=False)

def run_game_webview(project_path_for_game: str):
    """Run the game in a webview window."""
    # Get game title and icon from project configuration for loading window
    game_title = "Game"
    icon_path = None
    
    try:
        project_config_path = os.path.join(project_path_for_game, 'project.json')
        if os.path.exists(project_config_path):
            with open(project_config_path, 'r') as f:
                ...
                if icon_relative_path:
                    icon_absolute_path = os.path.join(project_path_for_game, icon_relative_path)
    ...
    def game_startup_sequence():
        """The game startup sequence that runs behind the loading window."""
        # Set the game project path in the game server
        set_game_project_path(project_path_for_game)

        # Start the game server in a separate thread
        game_thread = threading.Thread(target=start_game_server)
        game_thread.daemon = True  # Daemonize thread so it exits when main thread exits
        game_thread.start()

        # Give the game server a moment to start up
        time.sleep(2) 
        
        return True  # Signal that game startup is complete
    
    # Run the game startup sequence with the loading window
    loading_window.run_with_loading(game_startup_sequence)
    
    # Create webview window (this must run on main thread after loading window closes)
    # The URL should match the game server's host and port
    webview.create_window(game_title, 'http://127.0.0.1:5001', width=1024, height=768)
    webview.start()
